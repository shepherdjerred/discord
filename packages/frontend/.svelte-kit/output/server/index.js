...
function coalesce_to_error(err) {
  return err instanceof Error || (err && /** @type {any} */ err.name && /** @type {any} */ err.message)
    ? /** @type {Error} */
      err
...
function is_action_json_request(event) {
  const accept = negotiate(event.request.headers.get("accept") ?? "*/*", ["application/json", "text/html"]);
...
function validate_action_return(data) {
  ...
  if (data instanceof HttpError) {
    throw new Error(`Cannot \`return error(...)\` — use \`throw error(...)\` or \`return fail(...)\` instead`);
  }
}
...
function serialize_data(fetched, filter, prerendering = false) {
  ...
  const attrs = ['type="application/json"', "data-sveltekit-fetched", `data-url=${escape_html_attr(fetched.url)}`];
    ...
    for (let i2 = 0; i2 < 64; i2++) {
      ...
      out0 =
        (tmp +
          ...
          ((out1 >>> 2) ^ (out1 >>> 13) ^ (out1 >>> 22) ^ (out1 << 30) ^ (out1 << 19) ^ (out1 << 10))) |
        0;
    }
    out[0] = (out[0] + out0) | 0;
    out[1] = (out[1] + out1) | 0;
...
class BaseProvider {
  ...
  constructor(use_hashes, directives, nonce) {
    ...
    this.#script_needs_csp =
      !!effective_script_src && effective_script_src.filter((value) => value !== "unsafe-inline").length > 0;
    this.#style_needs_csp =
      !!effective_style_src && effective_style_src.filter((value) => value !== "unsafe-inline").length > 0;
...
async function render_response({
  ...
  let rendered;
  const form_value =
    action_result?.type === "success" || action_result?.type === "failure" ? action_result.data ?? null : null;
  let base$1 = base;
  let assets$1 = assets;
  ...
  for (const dep of fonts) {
    ...
    if (resolve_opts.preload({ type: "font", path })) {
      ...
      const attributes = ['rel="preload"', 'as="font"', `type="font/${ext}"`, `href="${path}"`, "crossorigin"];
      head += `
  ...
  if (page_config.ssr && page_config.csr) {
    body += `
			${fetched
        .map((item) => serialize_data(item, resolve_opts.filterSerializedResponseHeaders, !!state.prerendering))
        .join("\n			")}`;
  }
  if (page_config.csr) {
    const included_modulepreloads = Array.from(modulepreloads, (dep) => prefixed(dep)).filter((path) =>
      resolve_opts.preload({ type: "js", path })
    );
...
async function respond_with_error({ event, options: options2, manifest, state, status, error: error2, resolve_opts }) {
  const fetched = [];
  try {
    const branch = [];
...
`);
  ...
  try {
    const strings = nodes.map((node) => {
      ...
      return `{"type":"data","data":${devalue.stringify(node.data, reducers)},${stringify_uses(node)}${
        node.slash ? `,"slash":${JSON.stringify(node.slash)}` : ""
      }}`;
    });
    return {
...
const valid_page_server_exports = ["load", "prerender", "csr", "ssr", "actions", "trailingSlash", "config"];
...
async function respond(request, options2, manifest, state) {
  ...
  if (options2.csrf_check_origin) {
    const forbidden =
      request.method === "POST" && request.headers.get("origin") !== url.origin && is_form_content_type(request);
    if (forbidden) {
  ...
  try {
    ...
    const { cookies, new_cookies, get_cookie_header } = get_cookies(request, url, trailing_slash ?? "never");
    ...
    if (response.status === 200 && response.headers.has("etag")) {
      ...
      if (if_none_match_value === etag) {
        ...
        for (const key2 of ["cache-control", "content-location", "date", "expires", "vary", "set-cookie"]) {
